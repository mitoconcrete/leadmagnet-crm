...
- 이미지: `node:22-alpine`, pnpm 10은 `corepack enable`로 고정(`package.json`의 `packageManager` 필드). api Dockerfile은 컨텍스트가 저장소 루트인 다단계 빌드(deps → build → runner). web은 `output: 'standalone'`.
