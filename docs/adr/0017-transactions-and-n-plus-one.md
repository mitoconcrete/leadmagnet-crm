...
- 알려진 예외: 템플릿 hard delete는 "참조 폼 수 조회 → DELETE" 두 문장이며 그 사이에 폼이 생기면 FK 위반으로 500이 난다. 운영자 한두 명이 같은 템플릿을 동시에 만지는 경우라 수용하되, 발생하면 409로 매핑하는 것이 다음 단계다.
...
- 회귀 방지: e2e 유틸 `countQueries(fn)`이 TypeORM 로거로 실행된 SQL 수를 세고, 대표 목록 엔드포인트가 데이터 N개에 대해 상한 안에 있음을 단언한다. 데이터가 5개일 때와 1개일 때 쿼리 수가 같아야 한다. 상한은 **비즈니스 쿼리 기준**이며 AuthGuard 세션 검증 1개와 BEGIN/COMMIT/ROLLBACK은 제외한다: 캠페인 목록 1, 캠페인 상세 2, 폼 목록 1, 폼 상세 2, 명단 2, 캠페인 성과 3, 채널 성과 2. 세션 검증 자체도 1쿼리(세션+운영자 JOIN)로 유지한다.
